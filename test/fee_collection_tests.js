...
describe("Xoc System Tests - Fee collection", function () {
    ...
    it("should check user has fee debt and treasury collected fee", async () => {
        const {xoc, reservehouse, weth, coinhouse, accountant, bid, treasury} = await loadFixture(setupFixture);

        const user = accounts[1];
        await weth.connect(user).deposit({value: DEPOSIT_AMOUNT});
        await weth.connect(user).approve(await reservehouse.getAddress(), DEPOSIT_AMOUNT);
        await reservehouse.connect(user).deposit(DEPOSIT_AMOUNT);
        await coinhouse.connect(user).mintCoin(await weth.getAddress(), await reservehouse.getAddress(), MINT_AMOUNT);
    ...
    it("should check setting zero address treasury fails", async () => {
        ...
        await expect(coinhouse.setTreasury(ethers.ZeroAddress)).to.be.revertedWithCustomError(
            coinhouse,
            "HouseOfCoin_invalidInput",
        );
    });

    it("should check setting treasury by non-owner fails", async () => {
        const {coinhouse} = await loadFixture(setupFixture);
        await expect(coinhouse.connect(accounts[1]).setTreasury(accounts[2].address)).to.be.reverted;
    });
    ...
    it("should check setting reserve minting fee by non-owner fails", async () => {
        ...
        await expect(reservehouse.connect(accounts[1]).setReserveMintFee(0)).to.be.reverted;
    });
    ...
    it("should check setting reserve minting fee to 100% fails", async () => {
        const {reservehouse} = await loadFixture(setupFixture);
        await expect(reservehouse.setReserveMintFee(ALL_BPS)).to.be.revertedWithCustomError(
            reservehouse,
