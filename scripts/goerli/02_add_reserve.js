...
const deploySystemContracts = async () => {
    ...
    await setUpHouseOfReserve(reservehouse, RESERVE_CAPS.wbtc.defaultInitialLimit);

    await setUpOraclesHouseOfReserve(reservehouse, ethers.ZeroAddress, CHAINLINK_CONTRACTS.goerli.btcusd);

    const stx2 = await accountant.registerHouse(await reservehouse.getAddress());
    await stx2.wait();
